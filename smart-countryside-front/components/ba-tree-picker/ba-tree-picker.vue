          ...
          <block v-for="(item, index) in treeList" :key="index">
            ...
            >
              <view class="item-label">
                ...
                >
                  <view class="item-name" @tap.stop="_onItemSwitch(item, index)">
                    {{
                      item.name +
...
export default {
  ...
  methods: {
    ...
    _show(code) {
      ...
      console.log(code,'code',this.treeList);
      let item  = this.recursion(this.treeList,code)
      console.log(item);
    },
    _hide() {
    ...
    _confirm2(){
        this.$emit("select-change", this.selectedList, this.selectedNames);
        this._hide();
    },
    //格式化原数据（原数据为tree结构）
    ...
    _onItemSelect(item, index) {
      console.log('_onItemSelect选择',item)
      //console.log(item)
      if (!this.multiple) {
        //单选

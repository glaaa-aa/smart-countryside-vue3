...
## 注意
	作者不介意你对组件源码进行改造使用，为了节省不必要的沟通浪费，和作者怕麻烦；如果你对源码进行了修改使用，请不需要对作者做任何的反馈，作者确实没有空陪你做技术分析解答；为了开源更加高效，谢谢你的配合；
...
<template>
	...
	<next-tree :changeVerify="changeVerify" :title="getTitle" ref="nextTreeRef" :checkStrictly="checkStrictly" :selectParent="selectParent" :multiple="multiple" :treeData="treeData"  @cancel="oncancel" @confirm="onconfirm">
		<!-- label插槽示意代码 -->
...
<template>
	...
	<next-tree :changeVerify="changeVerify" :title="getTitle" ref="nextTreeRef" :checkStrictly="checkStrictly" :selectParent="selectParent" :multiple="multiple" :treeData="treeData" @cancel="oncancel" @confirm="onconfirm">
		<!-- label插槽示意代码 -->
		<!-- <template v-slot:label="{data: {id, label, iconSrc, prev, post}}">
			<view class="line-block">
				<image class="img" v-if="iconSrc" :src="iconSrc"></image>
				<text space="nbsp" v-if="prev">{{prev}}&nbsp;</text><text>{{label}}</text><text space="nbsp" v-if="post">&nbsp;{{post}}</text>
			</view>
...
|          增加验证函数和topBar插槽使得更加容易和组件进行交互    	         |
| :------------------------------------------------------------------: |
| ![](https://lixueshiaa.github.io/webtest/www/static/next-tree-cc.gif)|
## 参数
可选参数属性列表
...
|childrenKey	|子节点绑定属性																																																	                           |String	          |否				|children	|-			        |
...
|height| 弹层容器的高度，默认是500																																										                                     |Number            |否				|500		  |-		          |
|changeVerify| 验证函数  (a as any []):String => {} // 验证函数会把当前控件的选择值作为参数返给函数体，demo有说明																						 |Function          |否				|-		    |-		          |

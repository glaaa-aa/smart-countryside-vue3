    ...
    >
      ...
      >
        <view class="uni-file-picker__item">
          ...
          <view class="files__name" @click="click(item)">{{ item.name }}</view>
          <view
...
export default {
  name: "uploadFile",
  emits: ["uploadFiles", "choose", "delFile", "click"],
  props: {
    filesList: {
      type: Array,
      default() {
  ...
  methods: {
    ...
    click(item) {
      ...
      if (item.url.includes("http://")) {
        ...
        var a2 = item.url.replace(reg2, "");
        link = this.$mConfig.baseUrl + "/" + a2;
      } else {
        link = this.$mConfig.baseUrl + "/" + item.url;
      }
      uni.showLoading({
        title: "加载中",
      });

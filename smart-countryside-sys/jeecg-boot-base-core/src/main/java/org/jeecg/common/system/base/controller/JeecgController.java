...
public class JeecgController<T, S extends IService<T>> {
     ...
     * @description:
     */
    public QueryWrapper<T> beforeExport(HttpServletRequest request, QueryWrapper<T> wrapper){
        String selections = request.getParameter("selections");
        // 如果指定导出数据,则缩小查询范围
        if (StringUtils.isNotBlank(selections)){
            wrapper.in("id", Arrays.asList(selections.split(",")));
        }
        // 对QueryWrapper进行组装更多的查询条件
        return wrapper;
    }

    /**
     * 在导入的数据保存到数据库之前，可以先作一些处理逻辑
     * @param list
     * @return: void
     * @author: lanls
     * @date: 2022/11/14 0:28
     * @description:
     */
    public List<T> beforeImport(List<T> list){
        log.info("开始入库前，先作数据预处理....");
     ...
     * @description:
     */
    public void afterImport(List<T> list){
        log.info("完成数据入库后，对入库结果进一步处理....");
    }
     ...
     * @description:
     */
    protected ModelAndView exampleExportXls(List<T> exportList, Class<T> clazz, String title) {
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        // Step.3 AutoPoi 导出Excel
        ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
        //此处设置的filename无效 ,前端会重更新设置一下

...
public interface SysThirdAccountMapper extends BaseMapper<SysThirdAccount> {
     ...
     * @return
     */
    @Select("select MAX(CAST( SUBSTRING(serial_no,#{prefixLen}) as SIGNED)) as max from sys_third_account where serial_no like CONCAT(#{prefix,jdbcType=VARCHAR},'%')")
    public Integer getMaxSerialNo(@Param("prefix") String prefix,@Param("prefixLen") Integer prefixLen);
}

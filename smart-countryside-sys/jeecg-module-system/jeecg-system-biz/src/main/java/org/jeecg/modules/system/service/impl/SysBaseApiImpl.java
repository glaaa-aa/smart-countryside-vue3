...
public class SysBaseApiImpl implements ISysBaseAPI {
	...
	@Override
	public LoginUser getThirdAccountByThirdUser(String thirdUserId, String thirdType) {
		return null;
	}
...
//		loginUser.setPassword(null);
	...
	public void sendSysAnnouncement(MessageDTO message) {
		...
		try {
			// 同步发送第三方APP消息
			thirdAppModuleService.sendMessage(message, true,message.getTenantId());
		} catch (Exception e) {
	...
	public void sendTemplateAnnouncement(TemplateMessageDTO message) {
		...
		try {
			// 同步企业微信、钉钉的消息通知
//			thirdAppModuleService.sendDingTalkActionCardMessage(announcement, mobileOpenUrl, true);
//			thirdAppModuleService.sendWeChatTextCardMessage(announcement, true);
		} catch (Exception e) {
			log.error("同步发送第三方APP消息失败！", e);
		}
...
//			}
	...
	/**======================================================================================**/
	@Override
	public List<LoginUser> getUserByTenantId(Integer tenantId) {
		QueryWrapper<SysUser> queryWrapper = new QueryWrapper();
		queryWrapper.eq("tenant_id",tenantId);
		...
		List<LoginUser> resultList = new ArrayList<>();
	...
	public Object executeRule(String ruleCode, JSONObject formData) {
		return FillRuleUtil.executeRule(ruleCode,formData);
	}

	@Override
	public List<LoginUser> getUserByDepId(String departId, String username) {
		Page page = new Page();
		page.setSize(20);
		IPage<SysUser> pages = userMapper.getUserByDepId(page,departId,username);
	...
	@Override
	public SysDepartModel getDepartByCode(String orgCode) {
		if(StringUtils.isEmpty(orgCode)){
			return null;
		}
		SysDepart depart = departMapper.queryByOrgCode(orgCode);
		if(depart == null){
			return null;
		}
		SysDepartModel model = new SysDepartModel();
	...
	@Override
	public List<LoginUser> queryUsersByRealname(String[] realnames) {
		QueryWrapper<SysUser> queryWrapper = new QueryWrapper<SysUser>().eq("status",1).eq("del_flag",0);
		queryWrapper.in("realname",realnames);
		List<LoginUser> loginUsers = new ArrayList<>();
		List<SysUser> sysUsers = userMapper.queryPositionsByRealname(queryWrapper);
		for (SysUser user:sysUsers) {
			LoginUser loginUser = new LoginUser();
			BeanUtils.copyProperties(user, loginUser);
			loginUsers.add(loginUser);
	...
	@Override
	public LoginUser queryByThirdId(String thirdUserId, String thirdType) {
		SysThirdAccount account = thirdAccountService.getOneByThirdUserId(thirdUserId,thirdType);
		if(account == null) {
			return null;
		}
		if(StringUtils.isNotEmpty(account.getSysUserId())){
			LoginUser loginUser = getUserById(account.getSysUserId());
			if(loginUser != null){
				loginUser.setThirdUserId(thirdUserId);
				loginUser.setThirdType(thirdType);
				return loginUser;
			}
		}
		...
		loginUser.setId(account.getSysUserId());
...
//		loginUser.setPassword(account.getSerialNo());
	...
	@Override
	public LoginUser queryByCustomParams(QueryWrapper wrapper) {
		LoginUser loginUser = new LoginUser();
	...
	@Override
	public List<String> queryPermissionsByPrefix(String prefix) {
		QueryWrapper<SysPermission> wrapper = new QueryWrapper<>();
		wrapper.lambda().likeRight(SysPermission::getPerms, prefix);
	...
	@Override
	public void saveUser(LoginUser sysUser, String selectedRoles, String selectedDeparts) {
		try {

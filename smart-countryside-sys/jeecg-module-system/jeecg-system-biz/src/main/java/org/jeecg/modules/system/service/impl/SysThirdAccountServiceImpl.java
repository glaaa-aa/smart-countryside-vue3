...
public class SysThirdAccountServiceImpl extends ServiceImpl<SysThirdAccountMapper, SysThirdAccount> implements ISysThirdAccountService {
    ...
    public SysUser createUser(String phone, String thirdUserUuid, Integer tenantId) {
       //先查询第三方，获取登录方式
        ...
        user.setPhone(phone);
        user.setUserIdentity(60);
        user.setUserType(1);
        user.setSortOrder(100);
    ...
    public List<SysThirdAccount> listThirdUserIdByUsername(String[] sysUsernameArr, String thirdType) {
        return sysThirdAccountMapper.selectThirdIdsByUsername(sysUsernameArr, thirdType,null);
    }
    ...
    public SysThirdAccount saveThirdUser(ThirdLoginModel tlm) {
        ...
        user.setRealname(tlm.getUsername());
        user.setThirdUserUuid(tlm.getUuid());
        user.setThirdUserId(tlm.getThirdUserId());
        user.setUserType(tlm.getUserType());
        user.setPhone(tlm.getPhone());
        user.setPost(tlm.getPost());
        super.save(user);
        return user;
    }

    @Override
    public String getNextserial(String prefix) {
        Integer max = sysThirdAccountMapper.getMaxSerialNo(prefix,prefix.length() +1);
        if(max == null){
            max = 0;
        }
        max++;
        return prefix + String.format("%06d", max);
    }

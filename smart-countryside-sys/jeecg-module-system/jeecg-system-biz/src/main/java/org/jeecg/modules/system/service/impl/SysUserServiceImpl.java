...
//    ThirdAppDingtalkServiceImpl dingtalkService;
	...
	public boolean deleteUser(String userId) {
		...
		HrPerson hrPerson = personMapper.selectByPersonNo(sysUser.getUsername());
		LambdaUpdateWrapper<HrPerson> updateWrapper = new UpdateWrapper<HrPerson>().lambda();
		updateWrapper.set(HrPerson::getDelFlag, 1).eq(HrPerson::getId, hrPerson.getId());
		personMapper.update(new HrPerson(), updateWrapper);
		//1.删除用户
...
//			wechatEnterpriseService.removeThirdAppUser(userIds);
	...
	public void editUser(SysUser user, String roles, String departs, String relTenantIds) {
		...
		//根据账号编辑负责人员
		HrPerson hrPerson = personMapper.selectByPersonNo(user.getUsername());
		if(ObjectUtils.isNotEmpty(hrPerson)){
			hrPerson.setOrgCode(user.getDistrictCode());
			hrPerson.setDepartCode(user.getOrgCode());
			hrPerson.setCname(user.getRealname());
			hrPerson.setSex(user.getSex());
			hrPerson.setPhone(user.getPhone());
		}
		SysOrg sysOrg = orgMapper.queryByCode(user.getDistrictCode());
		if(ObjectUtils.isNotEmpty(sysOrg)){
			hrPerson.setOrgName(sysOrg.getName());
			hrPerson.setC1(sysOrg.getDescription());
		}

		SysDepart sysDepart = sysDepartMapper.queryByOrgCode(user.getOrgCode());
		if(ObjectUtils.isNotEmpty(sysDepart)){
			hrPerson.setDepartName(sysDepart.getDepartName());
		}

		personMapper.updateById(hrPerson);

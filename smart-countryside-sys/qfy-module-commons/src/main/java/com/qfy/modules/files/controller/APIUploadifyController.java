...
@Slf4j
@RestController
@RequestMapping("/api/uploadify")
public class APIUploadifyController {
...
//    private ISysBaseAPI sysBaseAPI;
    ...
    public Result<?> upload(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(StringUtils.isEmpty(name)){
           // 获取文件名
            oldName = CommonUtils.getFileName(file.getOriginalFilename());
        }else{
            oldName=CommonUtils.getFileName(name);
        }
        ...
        resultMap.put("name", savePath.substring(savePath.lastIndexOf("/") +1 ));
    ...
    public Result<?> uploadMulti(HttpServletRequest request, HttpServletResponse response) {
        ...
        for (MultipartFile mf : fileList){
            if(CommonConstant.UPLOAD_TYPE_LOCAL.equals(uploadType)){
                savePath = savePath + this.uploadLocal(mf,bizPath) +",";
            }else{
    ...
    private String uploadLocal(MultipartFile mf,String bizPath){
        try {
            ...
            if(orgName.indexOf(".")!=-1){
                fileName =  DateUtils.date2Str(DateUtils.yymmddhhmmssSSS.get()) + orgName.substring(orgName.lastIndexOf("."));
            }else{
                fileName = orgName+ "" + System.currentTimeMillis();
            }

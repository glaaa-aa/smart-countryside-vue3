...
import com.qfy.modules.smartform.entity.CheckPoint;
...
@Api(tags="巡检点（检查点）")
@RestController
@RequestMapping("/smartform/checkPoint")
@Slf4j
public class CheckPointController extends JeecgController<CheckPoint, ICheckPointService> {
	 ...
	 * @return
	 */
	@AutoLog(value = "巡检点（检查点）-分页列表查询")
	@ApiOperation(value="巡检点（检查点）-分页列表查询", notes="巡检点（检查点）-分页列表查询")
	@GetMapping(value = "/list")
	public Result<?> queryPageList(CheckPoint checkPoint,
	 ...
	 * @return
	 */
	@AutoLog(value = "巡检点（检查点）-添加")
	@ApiOperation(value="巡检点（检查点）-添加", notes="巡检点（检查点）-添加")
	@PostMapping(value = "/add")
	public Result<?> add(@RequestBody CheckPoint checkPoint) {
	 ...
	 * @return
	 */
	@AutoLog(value = "巡检点（检查点）-编辑")
	@ApiOperation(value="巡检点（检查点）-编辑", notes="巡检点（检查点）-编辑")
	@PutMapping(value = "/edit")
	public Result<?> edit(@RequestBody CheckPoint checkPoint) {
	 ...
	 * @return
	 */
	@AutoLog(value = "巡检点（检查点）-通过id删除")
	@ApiOperation(value="巡检点（检查点）-通过id删除", notes="巡检点（检查点）-通过id删除")
	 ...
	 * @return
	 */
	@AutoLog(value = "巡检点（检查点）-批量删除")
	@ApiOperation(value="巡检点（检查点）-批量删除", notes="巡检点（检查点）-批量删除")
	 ...
	 * @return
	 */
	@AutoLog(value = "巡检点（检查点）-通过id查询")
	@ApiOperation(value="巡检点（检查点）-通过id查询", notes="巡检点（检查点）-通过id查询")
    ...
    */
    ...
    public ModelAndView exportXls(HttpServletRequest request, CheckPoint checkPoint) {
        return super.exportXls(request, checkPoint, CheckPoint.class, "巡检点（检查点）");
    }

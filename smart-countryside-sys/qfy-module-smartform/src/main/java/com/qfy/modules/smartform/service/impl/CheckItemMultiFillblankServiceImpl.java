...
import com.qfy.modules.smartform.mapper.CheckItemMultiFillblankMapper;
 ...
 */
@Service
public class CheckItemMultiFillblankServiceImpl extends ServiceImpl<CheckItemMultiFillblankMapper, CheckItemMultiFillblank> implements ICheckItemMultiFillblankService {
    ...
    public List<CheckItemMultiFillblank> findByQuId(String quId){
        List<CheckItemMultiFillblank> checkItemRadioList=baseMapper.getCheckItemMultiFillblankList(quId);
    ...
    public CheckItem addMultiFillblank(CheckItem checkItem){
        ...
        checkItemnew.setCheckType(String.valueOf(QuType.MULTIFILLBLANK.getIndex()));
        ...
        //处理项单项数据
        List<CheckItemMultiFillblank> quMultiFillblank=new ArrayList<CheckItemMultiFillblank>();
        for (CheckItemMultiFillblank checkItemMultiFillblank : checkItem.getQuMultiFillblanks()) {
            CheckItemMultiFillblank checkItemMultiFillblanknew=new CheckItemMultiFillblank();
            if (StringUtil.isNotEmpty(checkItemMultiFillblank.getId())){
                checkItemMultiFillblanknew=getById(checkItemMultiFillblank.getId());
            }
            checkItemMultiFillblanknew.setItemId(checkItemnew.getId());
            checkItemMultiFillblanknew.setOptionName(checkItemMultiFillblank.getOptionName());
            checkItemMultiFillblanknew.setOrderBy(checkItemMultiFillblank.getOrderBy());

            if (StringUtil.isEmpty(checkItemMultiFillblank.getCheckType())){
                checkItemMultiFillblank.setCheckType("NO");
            }
            checkItemMultiFillblanknew.setCheckType(checkItemMultiFillblank.getCheckType());

            if (StringUtil.isEmpty(checkItemMultiFillblank.getFraction())){
                checkItemMultiFillblank.setFraction("0");
            }
            checkItemMultiFillblanknew.setFraction(checkItemMultiFillblank.getFraction());
            if (StringUtil.isEmpty(checkItemMultiFillblank.getType())){
                checkItemMultiFillblank.setType("0");
            }
            checkItemMultiFillblanknew.setType(checkItemMultiFillblank.getType());
            if (StringUtil.isEmpty(checkItemMultiFillblank.getVisibility())){
                checkItemMultiFillblank.setVisibility("1");
            }
            checkItemMultiFillblank.setVisibility(checkItemMultiFillblank.getVisibility());
            saveOrUpdate(checkItemMultiFillblanknew);
            quMultiFillblank.add(checkItemMultiFillblanknew);
        }
        checkItemnew.setQuMultiFillblanks(quMultiFillblank);
    ...
    public void deleteMultiFillblank(String quItemId){
        CheckItemMultiFillblank quCheckbox =getById(quItemId);

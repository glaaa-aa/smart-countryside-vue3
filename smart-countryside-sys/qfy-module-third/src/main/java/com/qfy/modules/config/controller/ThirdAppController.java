...
public class ThirdAppController {
    ...
    @Autowired
    private IThirdConfigService appConfigService;
    ...
    public Result getEnabledType() {
        ...
        //查询当前租户下的第三方配置
        List<ThirdConfig> list = appConfigService.getThirdConfigListByThirdType(tenantId);
        //钉钉是否已配置
        boolean dingConfig = false;
        //企业微信是否已配置
        boolean qywxConfig = false;
        if(null != list && list.size()>0){
            for (ThirdConfig config:list) {
    ...
    public Result syncWechatEnterpriseUserToApp(@RequestParam(value = "ids", required = false) String ids) {
        ...
        ThirdConfig config = appConfigService.getThirdConfigByThirdType(tenantId, MessageTypeEnum.QYWX.getType());
        if (null != config) {
        //update-begin---author:wangshuai ---date:20230224  for：[QQYUN-3440]通过租户模式隔离 ------------
            SyncInfoVo syncInfo = wechatEnterpriseService.syncLocalUserToThirdApp(ids,tenantId);
...
//        return Result.error("企业微信同步功能已禁用");
    ...
    public Result syncWechatEnterpriseDepartToApp(@RequestParam(value = "ids", required = false) String ids) {
        ...
        ThirdConfig config = appConfigService.getThirdConfigByThirdType(tenantId, MessageTypeEnum.QYWX.getType());
        if (null != config) {
            SyncInfoVo syncInfo = wechatEnterpriseService.syncLocalDepartmentToThirdApp(ids,tenantId);
            if (syncInfo.getFailInfo().size() == 0) {
...
//        return Result.error("企业微信尚未配置,请配置企业微信");
    ...
    public Result syncDingtalkDepartToApp(@RequestParam(value = "ids", required = false) String ids) {
        //获取钉钉配置
        Integer tenantId = oConvertUtils.getInt(TenantContext.getTenant(),0);
        ThirdConfig config = appConfigService.getThirdConfigByThirdType(tenantId, MessageTypeEnum.DD.getType());
        if (null != config) {
            SyncInfoVo syncInfo = dingtalkService.syncLocalDepartmentToThirdApp(ids,tenantId);
            if (syncInfo.getFailInfo().size() == 0) {
                return Result.OK("同步成功", null);
            } else {
                return Result.error("同步失败", syncInfo);
...
//    }
    ...
    public Result syncDingtalkUserToApp(@RequestParam(value = "ids", required = false) String ids) {
        ...
        int tenantId = oConvertUtils.getInt(TenantContext.getTenant(), 0);
        //根据租户id和第三方类别获取租户数据
        ThirdConfig appConfig = appConfigService.getThirdConfigByThirdType(tenantId,MessageTypeEnum.DD.getType());
        if(null != appConfig){
            SyncInfoVo syncInfo = dingtalkService.syncLocalUserToThirdApp(ids,tenantId);
            if (syncInfo.getFailInfo().size() == 0) {
                return Result.OK("同步成功", syncInfo);
            } else {
                return Result.error("同步失败", syncInfo);
...
//    }
    ...
    public Result sendMessageTest(@RequestBody JSONObject params, HttpServletRequest request) {
        ...
        if (weChatType.toUpperCase().equals(app)) {
            ThirdConfig config = appConfigService.getThirdConfigByThirdType(tenantId, weChatType);
            if (null != config) {
        //update-end---author:wangshuai ---date:20230224  for：[QQYUN-3440]钉钉、企业微信通过租户模式隔离 ------------
                JSONObject response = wechatEnterpriseService.sendMessageResponse(message, false,tenantId);
                return Result.OK(response);
            }
            return Result.error("企业微信尚未配置,请配置企业微信");
            //update-begin---author:wangshuai ---date:20230224  for：[QQYUN-3440]钉钉、企业微信通过租户模式隔离 ------------
        } else if (dingType.toUpperCase().equals(app)) {
            ThirdConfig config = appConfigService.getThirdConfigByThirdType(tenantId, dingType);
            if (null != config) {
            //update-end---author:wangshuai ---date:20230224  for：[QQYUN-3440]钉钉、企业微信通过租户模式隔离 ------------
                Response<String> response = dingtalkService.sendMessageResponse(message, false,tenantId);
                return Result.OK(response);
            }
            return Result.error("钉钉尚未配置,请配置钉钉");
        }
    ...
    public Result recallMessageTest(@RequestBody JSONObject params) {
        ...
        if (CommonConstant.WECHAT_ENTERPRISE.equals(app)) {
            ...
            ThirdConfig config = appConfigService.getThirdConfigByThirdType(tenantId, MessageTypeEnum.DD.getType());
            if (null != config) {
                Response<JSONObject> response = dingtalkService.recallMessageResponse(msgTaskId,tenantId);
    ...
    public Result<String> addThirdAppConfig(@RequestBody ThirdConfig appConfig) {
        ...
        ThirdConfig config = appConfigService.getThirdConfigByThirdType(tenantId, appConfig.getThirdType());
        if (null != config) {
        ...
        //通过应用key获取第三方配置
        List<ThirdConfig> thirdAppConfigByClientId = appConfigService.getThirdAppConfigByClientId(clientId);
    ...
    public Result<String> editThirdAppConfig(@RequestBody ThirdConfig appConfig) {
        Result<String> result = new Result<>();
        ThirdConfig config = appConfigService.getById(appConfig.getId());
        if (null == config) {
    ...
    public Result<ThirdConfig> getThirdAppByTenantId(@RequestParam(name = "tenantId", required = false) Integer tenantId,
                                                           @RequestParam(name = "thirdType") String thirdType) {
        Result<ThirdConfig> result = new Result<>();
        LambdaQueryWrapper<ThirdConfig> query = new LambdaQueryWrapper<>();
        query.eq(ThirdConfig::getThirdType,thirdType);
        ...
        query.eq(ThirdConfig::getTenantId,tenantId);
        ThirdConfig sysThirdAppConfig = appConfigService.getOne(query);
...
//        ThirdConfig config = appConfigService.getThirdConfigByThirdType(tenantId, MessageTypeEnum.DD.getType());
//        if (null != config) {
//            SyncInfoVo syncInfo = dingtalkService.syncThirdAppDepartmentUserToLocal(tenantId);
//            if (syncInfo.getFailInfo().size() == 0) {
//                return Result.OK("同步成功", syncInfo);
//            } else {
//                return Result.error("同步失败", syncInfo);

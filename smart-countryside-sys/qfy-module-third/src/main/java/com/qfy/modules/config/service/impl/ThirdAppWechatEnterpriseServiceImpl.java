...
import com.qfy.modules.dingtalk.vo.thirdapp.JwDepartmentTreeVo;
...
public class ThirdAppWechatEnterpriseServiceImpl implements IThirdAppService {
     ...
     */
    public final String THIRD_TYPE = ThirdAppEnum.WECHAT_ENTERPRISE.getCode();

    @Override
    public String getAccessToken(Integer tenantId) {
        ThirdConfig thirdConfig = ThirdConfigFactory.getConfig(tenantId,THIRD_TYPE);
        String corpId = thirdConfig.getClientId();
        String secret = thirdConfig.getClientSecret();
    ...
    public String getAppAccessToken(Integer tenantId) {
        ThirdConfig thirdConfig = ThirdConfigFactory.getConfig(tenantId,THIRD_TYPE);
        String corpId = thirdConfig.getClientId();
        String secret = thirdConfig.getAgentAppSecret();

        // 如果没有配置APP秘钥，就说明是老企业，可以通用秘钥
        if (oConvertUtils.isEmpty(secret)) {
            secret = thirdConfig.getClientSecret();
        }
...
//                    }
    ...
    public boolean sendMessage(MessageDTO message, boolean verifyConfig,Integer tenantId) {
        JSONObject response = this.sendMessageResponse(message, verifyConfig,tenantId);
        if (response != null) {
    ...
    public JSONObject sendMessageResponse(MessageDTO message, boolean verifyConfig,Integer tenantId) {
        ThirdConfig thirdConfig = ThirdConfigFactory.getConfig(tenantId,THIRD_TYPE);
        if (verifyConfig && !thirdConfig.getIsEnabled()) {
            return null;
        }
        String accessToken = this.getAppAccessToken(tenantId);
        if (accessToken == null) {
            return null;
        }
        Text text = new Text();
        ...
        text.setText(entity);
        text.setAgentid(Integer.parseInt(thirdConfig.getAgentId()));
    ...
    public JSONObject sendTextCardMessage(SysAnnouncement announcement, boolean verifyConfig,Integer tenantId) {
        ...
        textCard.setAgentid(Integer.parseInt(thirdConfig.getAgentId()));
    ...
    private SysUser getSysUserByThird(SysThirdAccount thirdAccount, User appUser, String appUserId, String accessToken) {
        ...
        if (oConvertUtils.isNotEmpty(sysUserId)) {
            ...
            if (sysUser != null) {
                ...
                // 没有就走创建逻辑
                return sysThirdAccountService.createUser(appUser.getMobile(), appUser.getUserid(),thirdAccount.getTenantId());
            }

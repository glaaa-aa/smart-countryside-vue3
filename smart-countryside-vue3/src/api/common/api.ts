...
enum Api {
  ...
  getemberList = '/family/familyMember/list?familyRel=1',
  getHrPersonList = '/info/hrPerson/list',
  getCheckItemList = '/smartform/checkItem/list?column=name&order=asc',
  getCheckPositionList = '/address/list?addType='
}
...
export const uploadUrl = `${baseUploadUrl}/sys/uploadify/upload`; //使用新的文件上传
/**
 * 职务列表
 * @param params
 */
 ...
 * @param params
 */
export const getFamilyMemberList = (params) => {
  return defHttp.get({ url: Api.getemberList, params });
};
/**
 * 人员列表
 * @param params
 */
export const getHrPersonList = (params) => {
  return defHttp.get({ url: Api.getHrPersonList, params });
};
/**
 * 加分扣分事项
 * @param params
...
export const getCheckItemList = (params) => {
  return defHttp.get({ url: Api.getCheckItemList, params });
};
/**
 * 地图定位坐标地址（id）
 * @param params
 */
export const getCheckPositionList = (params) => {
  return defHttp.get({ url: Api.getCheckPositionList + (localStorage.getItem('urlType') || ''), params });
};
